...
<?php
/**
 * Plugin Name: Hotel Reservation – Ultimate (Room Types, Quantity, Seasonal + Weekly/Monthly, Min/Max Nights, Closed-out, Stripe)
 * Description: Room types with quantity, seasonal pricing, weekly/monthly rates, min/max nights per season, closed-out dates, overlap-aware availability, Stripe payments, and shortcodes.
 * Version: 1.5.0
 * Author: M365 Copilot
...
if (!defined('HRT_PLUGIN_VERSION')) define('HRT_PLUGIN_VERSION', '1.5.0');
...
class HRT_Plugin {
    ...
    public function init() { HRT_Post_Types::register(); }
    ...
    public function shortcode_search($atts = [], $content = '') {
        $q = new WP_Query(['post_type'=>'hrt_room_type','posts_per_page'=>-1,'post_status'=>'publish']);
        ob_start(); ?>
        <div class="hrt-search">
            <form class="hrt-inline-form" onsubmit="return false;">
                <label><?php esc_html_e('Check-in', 'hotel-reservation-lite'); ?>
                    <input type="date" class="hrt-checkin" min="<?php echo esc_attr(date('Y-m-d')); ?>" />
                </label>
                <label><?php esc_html_e('Check-out', 'hotel-reservation-lite'); ?>
    ...
    public function shortcode_booking($atts = [], $content = '') {
        $atts = shortcode_atts(['room_type_id'=>0], $atts, 'hotel_booking');
        $room_type_id = absint($atts['room_type_id']); if(!$room_type_id) { $room_type_id = get_the_ID(); }
        if (!$room_type_id || get_post_type($room_type_id) !== 'hrt_room_type') { return '<p>'.esc_html__('Invalid room type.', 'hotel-reservation-lite').'</p>'; }
        ...
        <div class="hrt-booking" data-room-type-id="<?php echo esc_attr($room_type_id); ?>">
            ...
            <form class="hrt-booking-form" onsubmit="return false;">
                <div class="hrt-form-grid">
                    ...
                    <label><?php esc_html_e('Guest name', 'hotel-reservation-lite'); ?>
                        <input type="text" name="guest_name" required>
                    </label>
                    <label><?php esc_html_e('Email', 'hotel-reservation-lite'); ?>
                        <input type="email" name="guest_email" required>
                    </label>
                    <label><?php esc_html_e('Phone (optional)', 'hotel-reservation-lite'); ?>
